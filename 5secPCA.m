...
for k = 1:length(files)
    ...
    if record_length >= 5 && record_length <= 10
        ...
        for chunkIdx = 1:numChunks
            ...
            X_all = [X_all; double(specNorm(:)')];
            X_all = [X_all; double(specSped(:)')];
            X_all = [X_all; double(specSlowed(:)')];
...
% Apply PCA
targetSize = size(specSlowed);
[coeff, score, latent, ~, ~, ~] = pca(X_all);

% Normalize the score
whitened_score = score ./ sqrt(latent' + 1e-8);

% Reconstruct the flattened spectrogram, but now with withened features.
X_whitened = whitened_score*coeff.';

% Add the whitened spectrograms to the cell
numImgs = size(X_whitened, 1);
reconstructedSpectrograms = cell(numImgs/3, 1);
for i = 1:(numImgs/3)
    reconstructedSpectrograms{i} = cat(3, reshape(X_whitened(i*3-2,:), targetSize), reshape(X_whitened(i*3-1,:), targetSize), reshape(X_whitened(i*3,:), targetSize));
end

% Save results
save(fullfile(dataFolder, 'PCA_Augmented_Data.mat'), 'reconstructedSpectrograms', '-v7.3');
fprintf('Done!')
